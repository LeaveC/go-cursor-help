        ...
        for file in "${files[@]}"; do
            ...
            if [[ "$file" == *"extensionHostProcess.js"* ]]; then
                ...
                if grep -q "function a\$" "$file"; then
                    ...
                    # 生成唯一标识符
                    local unique_id="$((UNIQUE_ID_COUNTER++))"
                    inject_code="
// Cursor ID 修改工具注入 - $(date +%Y%m%d%H%M%S)
// 随机设备ID生成器注入 - ${unique_id}
const randomDeviceId_${unique_id} = () => {
    try {
...
"
                    ...
                    # 替换调用点
                    sed -i.tmp "s/await v5(!1)/randomDeviceId_${unique_id}()/g" "$file"
                    sed -i.tmp "s/a\$(t)/randomDeviceId_${unique_id}()/g" "$file"
...
// Cursor ID 修改工具注入 - $(date +%Y%m%d%H%M%S)
// 全局拦截设备标识符 - ${unique_id}
const originalRequire_${unique_id} = require;
require = function(module) {
    const result = originalRequire_${unique_id}(module);
    if (module === 'crypto' && result.randomUUID) {
        const originalRandomUUID_${unique_id} = result.randomUUID;
